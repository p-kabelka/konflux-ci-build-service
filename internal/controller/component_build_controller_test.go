...
var _ = Describe("Component build controller", func() {
	...
	Context("Test Pipelines as Code build preparation", func() {
		...
		It("should successfully submit PR with PaC definitions using GitHub application for non existing pipeline using bundle path", func() {
			mergeUrl := "merge-url"
			...
			annotationValue := fmt.Sprintf("{\"name\":\"%s\",\"bundle\":\"%s\"}", nonConfigMapPipelineName, nonConfigMapPipelineBundle)
			annotations := map[string]string{defaultBuildPipelineAnnotation: annotationValue}
			createCustomComponentWithBuildRequest(componentConfig{
				componentKey: resourcePacPrepKey,
				annotations:  annotations,
		...
		It("should successfully submit PR with PaC definitions using GitHub application, using name lookup in configMap", func() {
			mergeUrl := "merge-url"
			...
			annotationValue := fmt.Sprintf("{\"name\":\"%s\"}", defaultPipelineName)
			annotations := map[string]string{defaultBuildPipelineAnnotation: annotationValue}
			createCustomComponentWithBuildRequest(componentConfig{
				componentKey: resourcePacPrepKey,
				annotations:  annotations,
		...
		It("should successfully submit PR with PaC definitions using GitHub application, using git resolver by name in configMap", func() {
			mergeUrl := "merge-url"
			...
			annotationValue := fmt.Sprintf("{\"name\":\"%s\"}", gitPipelineName)
			annotations := map[string]string{defaultBuildPipelineAnnotation: annotationValue}
			createCustomComponentWithBuildRequest(componentConfig{
				componentKey: resourcePacPrepKey,
				annotations:  annotations,
