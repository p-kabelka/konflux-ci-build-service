...
var _ = Describe("Component build controller", func() {
	...
	Context("Test Pipelines as Code build preparation", func() {
		...
		It("should successfully submit PR with PaC definitions using GitHub application for non existing pipeline using bundle path", func() {
			...
			annotationValue := fmt.Sprintf("{\"name\":\"%s\",\"bundle\":\"%s\"}", "maven-zip-build-oci-ta", "quay.io/konflux-ci/tekton-catalog/pipeline-maven-zip-build-oci-ta:782e615430080b63ff60751f6ea597625e13aee3")
			annotations := map[string]string{defaultBuildPipelineAnnotation: annotationValue}
			createCustomComponentWithBuildRequest(componentConfig{
				componentKey: resourcePacPrepKey,
				annotations:  annotations,
